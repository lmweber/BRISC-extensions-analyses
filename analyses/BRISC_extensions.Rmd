...
# Extension 2: Re-use nearest neighbors array

The second possible extension we have identified is to re-use the nearest neighbors array.

The nearest neighbor array is currently calculated internally in `BRISC_estimation()`, so in our loop setup this is re-calculated in each iteration. If it is possible to calculate the nearest neighbors array once at the start and then pass this as an input in the loop iterations, this could potentially also save a large amount of runtime.
